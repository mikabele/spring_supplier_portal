...
public interface ProductRepository extends CrudRepository<ProductDomain, UUID> {
    ...
    @Query(value = "SELECT p.id, p.name, p.category_id, c.name, p.supplier_id, s.name, s.address, p.price, p.description, p.status,p.publication_date " +
            "FROM product AS p " +
            ...
            "AND (TO_TIMESTAMP(:endDate,'yyyy-MM-dd HH:mm:ss') is null or p.publication_date <= TO_TIMESTAMP(:endDate,'yyyy-MM-dd HH:mm:ss'))", nativeQuery = true)
    List<ProductDomain> searchByCriteria( String name,  String category,  String description,
                                          String supplier,  Float minPrice,  Float maxPrice,
